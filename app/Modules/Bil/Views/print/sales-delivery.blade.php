{{--
    Delivery note. Reproduces the legacy printout/sales_delivery.php as it
    ...
    THREE SIGNATURES, NOT TWO. The legacy sheet calls
    signature(true, 'Sent By', 'Received by Driver') — the driver signs for
    taking the goods and the CUSTOMER signs for receiving them, which is the
    whole difference between this sheet and the loading one. That third line is
    what makes it a delivery note rather than a picking list, so it is kept
    word for word.

    Like the loading sheet, it takes a LIST of deliveries and page-breaks
    between them, so a day's work prints in one go.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Delivery Print Out{{ count($deliveries) === 1 ? ' — ' . $deliveries[0]->barcode : '' }}</title>
...
<body>
    <div class="toolbar">
        <button type="button" onclick="window.print()">Print {{ count($deliveries) }} delivery note(s) again</button>
    </div>
        ...
        <div class="sheet">
            ...
            <table class="wrapper">
                <tr>
                    ...
                    <td class="title f-f-cursive">
                        <h2>Delivery</h2>
                        <small>number:</small> <span>{{ $delivery->deliverynumber }}</span> /
                        <small>date:</small> <span>{{ $delivery->dateofdelivery }}</span>
                    </td>
                </tr>
            </table>
            ...
            <div class="card f-f-cursive">
                <table class="table-grid">
                    <tr>
                        <td class="layout">
                            <p><label>Customer:</label> <span>{{ $delivery->customername ?: '—' }}</span></p>
                            <p><label>Sales Order(s):</label> <span>{{ implode(', ', $delivery->orders) ?: '—' }}</span></p>
                            <p><label>Load Number:</label> <span>{{ $delivery->loadbarcode ?: '—' }}</span></p>
                            <p><label>Warehouse Location:</label> <span>{{ $delivery->warehouse ?: '—' }}</span></p>
                        </td>
                        <td class="layout">
                            @if ($delivery->customeraddress)
                                <p><label>Customer Address:</label> <span>{{ $delivery->customeraddress }}</span></p>
                            @endif
                            <p><label>Transporter:</label> <span>{{ $delivery->transportername ?: '—' }}</span></p>
                            <p><label>Vehicle Number:</label> <span>{{ $delivery->trucknumber ?: '—' }}</span></p>
                            <p><label>Vehicle Driver:</label> <span>{{ $delivery->truckdriver ?: '—' }}</span></p>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <table class="lines">
                ...
                <tbody>
                    @foreach ($delivery->lines as $n => $line)
                        <tr>
                            <td>{{ $n + 1 }}</td>
                ...
                @if (count($delivery->lines) > 1)
                    <tfoot>
                        <tr>
                            <th colspan="3">TOTAL</th>
                            <th>{{ number_format($delivery->total) }}</th>
                        </tr>
                    </tfoot>
                @endif
            </table>
    ...
    <script type="text/javascript">
        ...
        var codes = @json(collect($deliveries)->pluck('barcode'));
